...
export default function Component() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <section className="flex-grow flex flex-col items-center justify-center bg-gray-100 p-4">
          ...
          {[...Array(MAX_ATTEMPTS)].map((_, rowIndex) => (
            <div key={rowIndex} className="grid grid-cols-5 gap-2">
              {[...Array(5)].map((_, colIndex) => {
                ...
                const letter = isCurrentGuessRow
                  ? currentGuess[colIndex]
                  : guesses[rowIndex]?.[colIndex] || "";
                const isRevealedGuess =
                  rowIndex === guesses.length - 1 && colIndex <= revealIndex;
                const color = isRevealedGuess
                ...
                return (
                  <motion.div
                    ...
                    className={`w-14 h-14 flex items-center justify-center text-2xl font-bold rounded border-2 ${
                      isCurrentGuessRow ? "border-gray-400" : "border-gray-200"
                    } ${color}`}
                    initial={isRevealedGuess ? { rotateX: 0 } : false}
                    animate={isRevealedGuess ? { rotateX: [0, 90, 0] } : {}}
        ...
        {gameStatus !== "playing" && (
          <Button
            onClick={resetGame}
            className="mb-4 bg-[#990000] hover:bg-[#800000] text-white"
          >
            Play Again
          </Button>
        )}
